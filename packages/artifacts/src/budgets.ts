...
export const BUDGETS: Record<BudgetTier, TierBudget> = {
  feed: {
    ...
    maxTextureBytes: 8_388_608,
    maxDrawCalls: 60,
  },
  fullscreen: {
    maxFileBytes: 12_582_912,
    maxTriangles: 600_000,
    maxTextureBytes: 33_554_432,
    maxDrawCalls: 200,
  },
  mobile: {
    maxFileBytes: 1_572_864,
    maxTriangles: 60_000,
    maxTextureBytes: 4_194_304,
    maxDrawCalls: 40,
  },
} as const;

export type BudgetRejectionCode =
  "glb_too_large" | "too_many_triangles" | "textures_too_large" | "too_many_draw_calls";
...
export function enforceBudget(tier: BudgetTier, m: SceneMetrics): void {
  ...
  if (m.fileBytes > b.maxFileBytes)
    throw new BudgetError("glb_too_large", m.fileBytes, b.maxFileBytes);
  if (m.triangles > b.maxTriangles)
    throw new BudgetError("too_many_triangles", m.triangles, b.maxTriangles);
  if (m.textureBytes > b.maxTextureBytes)
    throw new BudgetError("textures_too_large", m.textureBytes, b.maxTextureBytes);
  if (m.drawCallEstimate > b.maxDrawCalls)
    throw new BudgetError("too_many_draw_calls", m.drawCallEstimate, b.maxDrawCalls);
}
