...
const REMOTE_RE = /<script\b[^>]*\bsrc\s*=\s*['"]https?:|<link\b[^>]*rel\s*=\s*['"]?stylesheet['"]?[^>]*\bhref\s*=\s*['"]https?:|import\s*\(\s*['"]https?:/i;
...
export async function analyze2DBundle(
  ...
  for (const path of files.keys()) {
    if (path.startsWith('/') || path.includes('..')) return { rejection: { code: 'path_traversal', path } };
  }

  const manifestBytes = files.get('artifact.json');
  if (manifestBytes === undefined) return { rejection: { code: 'manifest_invalid', path: 'artifact.json' } };
  ...
  for (const [path, body] of files) {
    if (integrity.files[path] !== sha256Hex(body)) return { rejection: { code: 'integrity_mismatch', path } };
  }
  ...
  for (const [path, body] of files) {
    ...
    if (!ALLOWED_EXTENSIONS.has(ext(path))) return { rejection: { code: 'extension_not_allowed', path } };
  }
  ...
  if (entryBytes.byteLength > BUNDLE_CAPS.maxEntryBytes) return { rejection: { code: 'entry_too_large', path: entry } };
