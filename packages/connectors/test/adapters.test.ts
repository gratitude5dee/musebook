...
describe("mcp_http client revision (2026-07-28)", () => {
  it("sends _meta, revision headers and no initialize / Mcp-Session-Id", async () => {
    ...
    const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
      ...
      return new Response(
        JSON.stringify({ jsonrpc: "2.0", id: "x", result: { resultType: "complete" } }),
        {
          status: 200,
    ...
    await mcpCall(ctx(fetchImpl), "https://mcp.example/mcp", SEED_CREDENTIAL, "tools/call", {
      name: "draft_post",
    });
  ...
  it("falls back to tools/list when server/discover is -32601", async () => {
    ...
    const manifest = manifestFor({ kind: "mcp_http", url: "https://mcp.example/mcp" }, [
      "post.draft",
      "feed.read",
    ]);
    const c = ADAPTERS.mcp_http({
      manifest,
...
describe("http_openapi schema discipline", () => {
  it("a response not matching the declared output schema is a typed failure", async () => {
    ...
    const manifest = manifestFor({ kind: "http_openapi", baseUrl: "https://api.example" }, [
      "post.draft",
    ]);
    const c = ADAPTERS.http_openapi({
      manifest,
      connectorRowId: "crow-2",
      transport: manifest.transports[0],
...
describe("bridge_token", () => {
    ...
    for (const o of ops) {
      expect(
        Object.keys(o).filter(
          (k): k is Forbidden => k === "exec" || k === "command" || k === "cmd",
        ),
      ).toEqual([]);
  ...
  it("draftPost submits — the far side may be asleep", async () => {
    ...
    const c = ADAPTERS.bridge_token({
      manifest,
      connectorRowId: "crow-3",
      transport: manifest.transports[0],
      credential: null,
    });
