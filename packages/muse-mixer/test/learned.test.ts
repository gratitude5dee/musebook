...
import { MuseScorer, candidateFeatures, viewerContextFor } from "../src/muse/scorers/muse_scorer.js";
...
describe("LearnedMuseRanker (§9.15 v1.1)", () => {
  it("is the same math as the heuristic on the same coefficients", async () => {
    ...
    const learned = await new LearnedMuseRanker("v1.model-learned", HEURISTIC_V1_COEFFICIENTS).predict(
      vc,
      feats,
    );
